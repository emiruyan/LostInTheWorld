...
namespace AlmostEngine.Screenshot
{
    public class ScreenshotConfigDrawer
    {
        ...
        SerializedProperty m_ShotPerSeconds;
        ...
        public void Init(SerializedObject s, Object obj, ScreenshotConfig config, SerializedProperty configProperty)
        {
            ...
            m_ShotPerSeconds = configProperty.FindPropertyRelative("m_ShotPerSeconds");
        ...
        */
        ...
        public virtual void DrawCaptureGUI()
        {
            ...
            if (m_Config.m_ShotMode == ScreenshotConfig.ShotMode.BURST)
            {
                ...
                EditorGUILayout.PropertyField(m_ShotPerSeconds);
                EditorGUI.indentLevel--;
            }
        ...
        public void DrawFeatureExclude()
        {
            ...
            {
                if (GUILayout.Button("Exclude from build tool: Enabled"))
                {
            ...
            else
            {
                var colorBk = GUI.color;
                GUI.color = Color.gray;
                if (GUILayout.Button("Exclude from build tool: Disabled"))
                {
            ...
            GUI.color = Color.gray;
            if (GUILayout.Button("InputSystem support : Disabled")) { InputSystemSupport.ToggleInputSystemSupport(true); }
            GUI.color = colorBBk;
#else
            if (GUILayout.Button("InputSystem support : Enabled")) { InputSystemSupport.ToggleInputSystemSupport(false); }
#endif
#endif
        }
        ...
        void DrawFeatureToggle(string name, string description, ref bool enabled, UnityAction<bool> onToggle)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button(name + (enabled ? " : Enabled " : " : Disabled")))
            {
                if (onToggle != null)
                {
                    onToggle(!enabled);
